    ...
    jint height)
{
    ...
    for (int y = 1; y < h - 1; ++y)
    {
        for (int x = 1; x < w - 1; ++x)
        {
            int gx = -gray[idxAt(x - 1, y - 1)] - 2 * gray[idxAt(x - 1, y)] - gray[idxAt(x - 1, y + 1)] + gray[idxAt(x + 1, y - 1)] + 2 * gray[idxAt(x + 1, y)] + gray[idxAt(x + 1, y + 1)];
            int gy = -gray[idxAt(x - 1, y - 1)] - 2 * gray[idxAt(x, y - 1)] - gray[idxAt(x + 1, y - 1)] + gray[idxAt(x - 1, y + 1)] + 2 * gray[idxAt(x, y + 1)] + gray[idxAt(x + 1, y + 1)];
